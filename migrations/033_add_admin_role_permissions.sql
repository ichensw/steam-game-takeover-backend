...
SET @add_admin_permissions_sql := (
  SELECT IF(COUNT(*) = 0, 'ALTER TABLE `ttw_admin_user` ADD COLUMN `permissions` json DEFAULT NULL COMMENT ''管理员权限列表'' AFTER `role`', 'SELECT 1')
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ttw_admin_user' AND COLUMN_NAME = 'permissions'
);
PREPARE add_admin_permissions_stmt FROM @add_admin_permissions_sql;
EXECUTE add_admin_permissions_stmt;
DEALLOCATE PREPARE add_admin_permissions_stmt;

UPDATE `ttw_admin_user`
SET `role` = 'super_admin', `permissions` = JSON_ARRAY()
